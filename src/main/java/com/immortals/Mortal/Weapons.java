...
public class Weapons {
    ...
    public static void register() {
        ...
        // Register ability activation using keybind
        ...
        UseItemCallback.EVENT.register((sp, world, hand) -> {
            ...
            if (inHand.getItem() == Items.DRAGON_EGG) {
                ...
                // If the player is mortal and uses the timekeeper, they unlock chronoreaver
            } else if (inHand.getItem() == ModItems.TIMEKEEPER) {
                inHand.decrement(1);
                player.getInventory().offerOrDrop(new ItemStack(ModItems.CHRONOREAVER));
                world.playSound(null, player.getX(), player.getY(), player.getZ(),
                        SoundEvent.of(Identifier.of("immortals", "glass")),
                        net.minecraft.sound.SoundCategory.PLAYERS, 1.0F, 1.0F);
        ...
        ServerLivingEntityEvents.AFTER_DAMAGE.register((entity, source, baseDamageTaken, damageTaken, blocked) -> {
            ...
            if (Utils.inventoryHas(player, ModItems.CHRONOREAVER) == -1)
                return;

            final UUID playerId = player.getUuid();
            float newHealth = entity.getHealth();
            float oldHealth = newHealth + damageTaken;
            float threshold = Main.CONFIG.getInt("blinkThreshold");
            Long last = BLINK_COOLDOWNS.get(playerId);
            // Check if blink is not on cooldown
            if ((last == null || System.currentTimeMillis() - last >= Main.CONFIG.getInt("blinkCooldown") * 50)
                    && oldHealth >= threshold
                    && newHealth < threshold) {
                int blinkDuration = Main.CONFIG.getInt("blinkDuration");
                ...
                player.sendMessage(Text.literal("Your existence begins to blink!"), true);
            }
        });
        ...
        AttackEntityCallback.EVENT.register((attacker, world, hand, victim, hitResult) -> {
            ...
            // block
            if (sp.getAttackCooldownProgress(0.5F) < 0.85F) {
                return ActionResult.PASS;
            }
            ...
            if (victim instanceof LivingEntity living && living.isBlocking()) {
                ...
                // dot > -1.0 means attacker is within 180 degrees in front
                if (dot > -1.0) {
                    return ActionResult.PASS;
                }
            }
            ...
            if (count >= Main.CONFIG.getInt("fractalEdgeHits")) {
                        ...
                        Main.scheduler.schedule(() -> {
                            ...
                            for (int j = 0; j < 3; j++) {
                                Main.scheduler.schedule(() -> {
                                    ...
                                    fractalEdgeAnimation(particlePos.x, particlePos.y, particlePos.z, sp);
        ...
        ServerTickEvents.START_SERVER_TICK.register(server -> {
            ...
            for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                ...
                } else if (OVERCLOCK_COOLDOWNS.get(id) != null
                        && player.getMainHandStack().getItem() == ModItems.CHRONOREAVER) {
                    long elapsed = System.currentTimeMillis() - OVERCLOCK_COOLDOWNS.get(id);
                ...
                } else if (BLINK_COOLDOWNS.get(id) != null
                        && player.getMainHandStack().getItem() == ModItems.CHRONOREAVER) {
                    long elapsed = System.currentTimeMillis() - BLINK_COOLDOWNS.get(id);
                ...
                // Check if the weapon owner has changed and transform it into a dragon egg
                if (((ImmortalsData) player).isImmortal()) {
                    var inv = player.getInventory();

                    int found = Utils.inventoryHas(player, ModItems.PHASEBREAKER);
                    if (found != -1) {
                        inv.setStack(found, new ItemStack(Items.DRAGON_EGG));
                        player.sendMessage(
    ...
    private static void fractalEdgeAnimation(double x, double y, double z, ServerPlayerEntity player) {
    ...
    public static boolean phaseChange(ServerPlayerEntity player, ServerWorld world) {
        ...
        if (last != null && now - last < Main.CONFIG.getInt("phaseChangeCooldown") * 50) {
            return false;
        }
    ...
    public static boolean overclock(ServerPlayerEntity player, ServerWorld world) {
        ...
        if (last != null && now - last < Main.CONFIG.getInt("overclockCooldown") * 50) {
            return false;
        }
        ...
        world.playSound(null, player.getX(), player.getY(), player.getZ(),
                ...
                net.minecraft.sound.SoundCategory.PLAYERS, 1.0F, 1.0F);
        player.addStatusEffect(new net.minecraft.entity.effect.StatusEffectInstance(
                net.minecraft.entity.effect.StatusEffects.SPEED, duration, 2)); // Speed 3
        player.addStatusEffect(new net.minecraft.entity.effect.StatusEffectInstance(
