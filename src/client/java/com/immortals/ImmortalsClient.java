...
import com.immortals.hud.SpellHudClient;
...
public class ImmortalsClient implements ClientModInitializer {
    private Map<String, Identifier> spellTextures = Map.of(
            ...
            "blackout", Identifier.of("immortals", "textures/quads/blackout.png"));

    /**
     * Key binding for activating a spell. Defaults to the "B" key.
     * Shift right clicking still always works for casting a spell.
     */
    ...
    private static record GhostData(Vec3d pos, float[] rgba, int lifetimeTicks,
            int ticksElapsed) {
    }
    ...
    public void onInitializeClient() {
        ...
        // Register network packet handlers from server
                ...
                (payload, context) -> {
                    ...
                    context.client().execute(() -> {
                        SpellHudClient.displaySpell(
                                payload.spellID(),
                                payload.state(),
                                payload.currCooldown(),
                                payload.maxCooldown());
                    });
                });

        // Render spell hud
        HudElementRegistry.attachElementBefore(VanillaHudElements.CHAT, Identifier.of("immortals", "spell_hud"),
                SpellHudClient::render);

        ClientPlayNetworking.registerGlobalReceiver(NetworkChannels.GhostS2CPayload.ID, (payload, context) -> {
            ClientWorld world = context.client().world;
            ...
            pendingObjects.add(new GhostData(
            ...
            for (Object o : pendingObjects) {
                ...
                if (o instanceof RuneData rune) {
                    ...
                    ClientUtils.renderItem(context, itemData.item(), x, y, z, end.x, end.y, end.z,
                            itemData.size());
                } else if (o instanceof GhostData ghost) {
                    ClientPlayerEntity player = MinecraftClient.getInstance().player;
                    if (player != null) {
                        ClientUtils.renderPlayerGhost(context, player, ghost.pos().x, ghost.pos().y, ghost.pos().z,
                                ghost.rgba());
                    }
                }
            }
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            for (int i = pendingObjects.size() - 1; i >= 0; i--) {
                ...
                if (o instanceof RuneData rune) {
                    ...
                    int newTicksElapsed = ghost.ticksElapsed() + 1;
                    if (ghost.lifetimeTicks() - newTicksElapsed <= 0) {
                        pendingObjects.remove(i);
                    } else {
                        pendingObjects.set(i, new GhostData(ghost.pos(), ghost.rgba(),
                                ghost.lifetimeTicks(), newTicksElapsed));
                    }
                }
            }
        });
